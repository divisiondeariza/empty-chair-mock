...
;
jQuery(function($){
	$('.comment-reply a').click(function(e){
		e.preventDefault();
		cid = $(this).parents('.answer').attr('comment');
		$('#form-responder-'+cid).fadeToggle();
	});
	/* Funciones para debate de la silla llena */
	//autosize($('.form-opinion textarea'));
	
	$('.elcomentario .more').click(function(e) {
	  e.preventDefault();
	  _this = $(this);
	  $(this).parent().find('span').remove();
	  $(this).parent().html($(this).parent().parent().find('.comment-rest').html());
	  $(this).remove();
	});
	setTimeout(cargarRespuestasDebate, 100);
	/* Fin funciones debate silla llena*/
	$( ".comment-form textarea" ).keypress(function() {
	  //console.log( $(this).attr('rows') );
	});
});

function cargarRespuestasDebate(){
	jQuery( ".comment" ).each(function( index ) {
		_this=jQuery(this);
		if(jQuery(this).find('.usuarios-respuesta').length!=0){
			jQuery('a#new').remove();
			fotos = jQuery(this).next('.indented').find('.img-author');
			if(fotos.length!=0){
				if(jQuery(this).find('.answer .list-inline').length!=0)
					jQuery(this).find('.answer .list-inline').append(jQuery('<li>').html('Responden:'));
				else
					jQuery(this).find('.answer').append(jQuery('<ul>').addClass('list-inline').html(jQuery('<li>').html('Responden:')));
			}
			jQuery.each(fotos, function( indexfoto, value ) {
			  jQuery(_this).find('.usuarios-respuesta').append(jQuery('<div>').addClass('follow-user col-sm-2').html(jQuery(value).html()));
			});
		}
	});
};
